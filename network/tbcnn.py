...
class TreebasedCNN():
    ...
    def __init__(self, num_types, num_tokens, num_labels, loss_type,
                num_conv: int=2, node_type_dim:int=50 , node_token_dim: int=50,
                conv_output_dim=50, include_token=1, main_batch_size=10, aux_batch_size=10, learning_rate: float=0.001):
    ...
    def init_net(self):
        ...
        with tf.compat.v1.name_scope('inputs'):
            ...
            # self.weights["language_embeddings"] = tf.Variable(tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")([self.num_languages, 1]), name='language_embeddings')
            ...
            self.placeholders["labels"] = tf.compat.v1.placeholder(tf.int32, shape=(None,), name="labels")
    ...
    def feed_forward(self):
        with tf.compat.v1.name_scope('network'):  
            ...
            # self.language_tensor_by_batch = tf.repeat(self.language_tensor, [tf.shape(input=self.parent_node_embeddings)[0]])
            
            # self.language_tensor_for_parent = tf.reshape(self.language_tensor_by_batch, [tf.shape(input=self.parent_node_embeddings)[0], 1, 1])
            # self.language_tensor_for_parent = tf.tile(self.language_tensor_for_parent, [1, tf.shape(input=self.parent_node_embeddings)[1], 1])
            # self.parent_node_embeddings = tf.concat([self.parent_node_embeddings, self.language_tensor_for_parent], 2)
            self.parent_node_embeddings = tf.compat.v1.layers.dense(self.parent_node_embeddings, units=self.node_dim, activation=tf.nn.leaky_relu, use_bias=False)

            # self.language_tensor_for_children= tf.reshape(self.language_tensor_by_batch, [tf.shape(input=self.parent_node_embeddings)[0], 1, 1, 1])
            # self.language_tensor_for_children = tf.tile(self.language_tensor_for_children, [1, tf.shape(input=self.children_embeddings)[1],  tf.shape(input=self.children_embeddings)[2], 1])
            # self.children_embeddings = tf.concat([self.children_embeddings, self.language_tensor_for_children], -1)
            self.children_embeddings = tf.compat.v1.layers.dense(self.children_embeddings, units=self.node_dim, activation=tf.nn.leaky_relu, use_bias=False)
            ...
            self.logits = tf.compat.v1.layers.dense(self.code_vector, self.num_labels, activation=tf.nn.leaky_relu)
            
            
            if self.loss_type == "softmax":
                self.LOGGER.info("Softmax loss")
                self.compute_softmax_loss()
            if self.loss_type == "oe":
                self.LOGGER.info("OE loss with auxiliary data")
                self.computer_OE_loss()
            if self.loss_type == "energy":
                self.LOGGER.info("Energy loss with auxiliary data")
                self.compute_energy_loss()
            ...
            with tf.control_dependencies(update_ops):
                self.training_point = self.optimizer.minimize(self.loss_avg)

    def compute_softmax_loss(self):
      
        self.softmax = tf.nn.softmax(self.logits)
        self.predicted_labels = tf.math.argmax(self.softmax, axis=1)
        self.softmax_prob = tf.compat.v1.reduce_max(self.softmax, reduction_indices=[1], keep_dims=False)
        self.energy = -tf.compat.v1.math.log(tf.compat.v1.math.reduce_sum(tf.compat.v1.math.exp(self.logits), axis=1))
        # self.kl_all = tf.compat.v1.math.log(2.) + tf.compat.v1.math.reduce_sum(self.softmax * tf.compat.v1.math.log(tf.compat.v1.math.abs(self.softmax) + 1e-10), reduction_indices=[1], keep_dims=True)
        # self.m_all, self.v_all = tf.nn.moments(self.kl_all, axes=[0])

        # self.logits_right = tf.boolean_mask(self.logits, tf.equal(tf.argmax(self.logits, 1), tf.cast(self.placeholders["labels"], tf.int64)))
        # self.softmax_right = tf.nn.softmax(self.logits_right)
        # self.softmax_right_prob = tf.compat.v1.reduce_max(self.softmax_right, reduction_indices=[1], keep_dims=True)

        # self.logits_wrong = tf.boolean_mask(self.logits, tf.not_equal(tf.argmax(self.logits, 1), tf.cast(self.placeholders["labels"], tf.int64)))
        # self.softmax_wrong = tf.nn.softmax(self.logits_wrong)
        # self.softmax_wrong_prob = tf.compat.v1.reduce_max(self.softmax_wrong, reduction_indices=[1], keep_dims=True)

        self.softmax_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.placeholders["labels"], logits=self.logits)
        self.loss_avg = tf.reduce_mean(input_tensor=self.softmax_loss)

    def compute_energy_loss(self):
        self.loss_avg = 0.0

        self.y_true = self.placeholders["labels"][:self.main_batch_size]
        self.y_predict = self.logits[:self.main_batch_size]

        self.softmax = tf.nn.softmax(self.logits[:self.main_batch_size])
        self.predicted_labels = tf.math.argmax(self.softmax, axis=1)
        self.nll_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y_true, logits=self.y_predict))
        # self.y_predict_softmax = tf.nn.softmax(self.y_predict)

        # energy = -torch.log(torch.sum(torch.exp(logits), dim=1))
        self.Ec_in = -tf.math.reduce_logsumexp(self.logits[:self.main_batch_size], axis=1)
        self.Ec_out = -tf.math.reduce_logsumexp(self.logits[self.main_batch_size:], axis=1)
        self.energy = tf.compat.v1.concat([self.Ec_in, self.Ec_out], -1)
        self.labels = tf.compat.v1.math.equal(self.placeholders["labels"], -1)
        
        self.energy_loss = 0.1*(tf.compat.v1.math.reduce_mean(tf.compat.v1.math.pow(tf.compat.v1.nn.relu(self.Ec_in-self.m_in), 2)) + tf.compat.v1.math.reduce_mean(tf.compat.v1.math.pow(tf.compat.v1.nn.relu(self.m_out-self.Ec_out), 2)))
        
        self.loss = self.nll_loss + self.energy_loss
        self.loss_avg = self.loss_avg * 0.8 + self.loss * 0.2
