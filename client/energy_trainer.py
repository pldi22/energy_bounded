...
from data_utils.auxiliary_data_loader import AuxiliaryDataLoader
...
class EnergyTrainer(BaseClient):

    LOGGER = logging.getLogger('SoftmaxTrainer')

    def __init__(self, language, 
                train_input_data_path, train_output_processed_data_path, 
                train_aux_input_data_path, train_aux_output_processed_data_path, 
                test_input_data_path, test_output_processed_data_path,
                test_aux_input_data_path, test_aux_output_processed_data_path):
        ...
        self.train_aux_input_data_path = train_aux_input_data_path
        self.train_aux_output_processed_data_path = train_aux_output_processed_data_path

        self.test_input_data_path = test_input_data_path
        self.test_output_processed_data_path = test_output_processed_data_path

        self.test_aux_input_data_path = test_aux_input_data_path
        self.test_aux_output_processed_data_path = test_aux_output_processed_data_path
    ...
    def init_from_config(self, config=None):
        ...
        self.training_data_processor = DatasetProcessor(input_data_path=self.train_input_data_path, 
                                                       ...
                                                       language=self.language, is_auxiliary=False, max_ast_size=1500)
        ...
        self.training_auxiliary_data_processor = DatasetProcessor(input_data_path=self.train_aux_input_data_path, 
                                                                  output_tensors_path=self.train_aux_output_processed_data_path, 
                                                                  node_type_vocab_model_prefix=self.node_type_vocab_model_prefix, 
                                                                  node_token_vocab_model_prefix=self.node_token_vocab_model_prefix, 
                                                                  language=self.language, is_auxiliary=True, max_ast_size=700)
        self.training_auxiliary_buckets = self.training_auxiliary_data_processor.process_or_load_data()


        # self.testing_auxiliary_data_processor = DatasetProcessor(input_data_path=self.test_aux_input_data_path, 
        #                                                          output_tensors_path=self.test_aux_output_processed_data_path, 
        #                                                          node_type_vocab_model_prefix=self.node_type_vocab_model_prefix, 
        #                                                          node_token_vocab_model_prefix=self.node_token_vocab_model_prefix, 
        #                                                          language=self.language, is_auxiliary=True, max_ast_size=700)
        # self.testing_auxiliary_buckets = self.testing_auxiliary_data_processor.process_or_load_data()

        # self.ast_util, self.training_buckets = self.process_or_load_data()        
        self.aux_data_loader = AuxiliaryDataLoader(self.batch_size, self.auxiliary_batch_size)
        ...
        self.tbcnn = TreebasedCNN(num_types=self.node_type_vocab.get_vocabulary_size(), 
                                  ...
                                  aux_batch_size=self.auxiliary_batch_size,
    ...
    def train(self):
        ...
        for epoch in range(1,  self.epochs + 1):
            train_batch_iterator = ThreadedIterator(self.aux_data_loader.make_minibatch_iterator(self.training_buckets, self.training_auxiliary_buckets), max_queue_size=5)
